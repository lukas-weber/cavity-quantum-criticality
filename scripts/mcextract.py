...
class MCArchive:
    def __init__(self, filename):
        ...
        observable_names = set(
            sum(
                [
                    list(task["results"].keys()) if task["results"] is not None else []
                    for task in doc
                ],
                [],
            )
        ...
        self.parameters = dict(
            zip(
                param_names,
                [[None for _ in range(self.num_tasks)] for _ in param_names],
            )
        )
        self.observables = dict(
            zip(
                observable_names,
                [Observable(self.num_tasks) for _ in observable_names],
            )
        )
    ...
    def filter_mask(self, filter):
        ...
        return [
            all(self.parameters[key][i] == val for key, val in filter.items())
            for i in range(self.num_tasks)
        ]

    def get_parameter(self, name, unique=False, filter={}):
        selection = list(
            itertools.compress(self.parameters[name], self.filter_mask(filter))
        )
        if len(selection) == 0:
